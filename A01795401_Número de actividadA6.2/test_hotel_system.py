...
class TestHotelSystem(unittest.TestCase):
    ...
    def test_delete_nonexistent_hotel(self):
        """Prueba la eliminación de un hotel que no existe."""
        Hotel.delete_hotel(999)  # ID inexistente
        hotels = Hotel.load_hotels()
        self.assertEqual(len(hotels), 0)  # No debe eliminar nada
    ...
    def test_modify_nonexistent_hotel(self):
        """Prueba la modificación de un hotel que no existe."""
        Hotel.modify_hotel(999, "Fake Hotel", "No Place", 5)  # ID inexistente
        hotels = Hotel.load_hotels()
        self.assertEqual(len(hotels), 0)  # No debe modificar nada
    ...
    def test_reserve_insufficient_rooms(self):
        """Prueba intentar reservar más habitaciones de las disponibles."""
        Hotel.create_hotel(8, "Hotel Pequeño", "Tacna", 2)
        result = Hotel.reserve_room(8, 5)  # Intentar reservar 5 habitaciones
        self.assertFalse(result)  # No debería permitir la reserva
    ...
    def test_delete_nonexistent_customer(self):
        ...
        Customer.modify_customer(999, "Fake Name", "fake@email.com")  # ID inexistente
        customers = Customer.load_customers()
        self.assertEqual(len(customers), 1)  # No debe afectar a los existentes
    ...
    def test_cancel_nonexistent_reservation(self):
        """Prueba intentar cancelar una reserva que no existe."""
        Reservation.cancel_reservation(999)  # ID inexistente
        reservations = Reservation.load_reservations()
        self.assertEqual(len(reservations), 0)  # No debe cambiar nada

    def test_load_invalid_json(self):
        """Prueba cargar un JSON corrupto."""
        with open(Hotel.FILE_PATH, "w", encoding="utf-8") as file:
            file.write("{")  # JSON inválido

        hotels = Hotel.load_hotels()
        self.assertEqual(len(hotels), 0)  # Debe manejar el error y devolver lista vacía
